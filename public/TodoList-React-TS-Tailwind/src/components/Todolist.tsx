...
function Todolist() {
  ...
  const toggleField = (id: string, field: keyof Pick<TodoItem, 'checked' | 'pinned' | 'recurring'>) => {
  ...
  const renderItem = (todo: TodoItem) => (
    ...
    >
      <div className="flex relative">
        <div className={`p-2 pr-14 bg-transparent break-words w-full font-medium ${todo.checked ? 'line-through decoration-2' : ''}`}>
      ...
      <div className="flex gap-2 p-2 justify-between">
        ...
        <div className="TodoIcons flex gap-4 justify-center items-center">
          ...
          <div
            ...
            onClick={e => { e.stopPropagation(); toggleField(todo.id, 'pinned'); }}
          >
            {todo.pinned ? <BsPinAngleFill /> : <BsPinAngle />}
          </div>
          <div
            className="p-1 rounded-md hover:scale-125 dark:text-slate-400 transition-all duration-300 hover:dark:text-slate-50 text-slate-700 hover:text-slate-950 cursor-pointer"
            onClick={e => { e.stopPropagation(); toggleField(todo.id, 'recurring'); }}
          >
          ...
          <div
            ...
            onClick={e => { e.stopPropagation(); removeItem(todo.id); }}
          >
            <IoIosCloseCircle />
          </div>
        </div>
  ...
  const renderList = () => {
    switch (filter) {
      case 'tags':     return renderByTags();
      case 'checked':  return renderByStatus('checked');
      case 'pinned':   return renderByStatus('pinned');
      case 'recurring': return renderByStatus('recurring');
      default:         return items.map(renderItem);
    }
  };
