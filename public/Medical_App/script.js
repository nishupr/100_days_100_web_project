document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        themeToggleBtn.addEventListener('click', () => {
            ...
            themeToggleBtn.innerHTML = isDark ? '<i class="ph ph-sun"></i>' : '<i class="ph ph-moon"></i>';
        });
        ...
        toggleAdvancedBtn.addEventListener('click', () => {
            ...
            const icon = advancedFields.classList.contains('hidden') ? 'ph-caret-down' : 'ph-caret-up';
            const text = advancedFields.classList.contains('hidden') ? 'Show Advanced Details' : 'Hide Advanced Details';
    ...
    function handleFileSelection(e) {
        ...
        Array.from(e.target.files).forEach(file => {
            ...
            item.innerHTML = `<i class="ph ph-file"></i> ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`;
            fileList.appendChild(item);
        });
    }

    function updateSpecialistOptions(searchText) {
        specialistTypeSelect.innerHTML = '<option value="">Select a Specialist...</option>';
        let foundAny = false;

        specialistCategories.forEach(group => {
            const filtered = group.specialists.filter(s => s.toLowerCase().includes(searchText.toLowerCase()));
            ...
            if (filtered.length > 0) {
                ...
                filtered.forEach(specialist => {
                    // Assign a stable random status based on string length to prevent it changing on every keypress
    ...
    // ==========================================================================
    // Form Handlers & Progress Tracker
    // ==========================================================================
    ...
    function handleRequestSubmit(e) {
        ...
        setTimeout(() => {
            ...
            document.getElementById('assignedSpecialistName').textContent = specialist.split(' - ')[0];
        }, 1000);
    ...
    function renderHistory() {
        ...
        consultationHistory.forEach((item, index) => {
            ...
            const priorityClass = item.priority ? `priority-${item.priority.toLowerCase()}` : 'priority-normal';
            ...
            li.innerHTML = `
                ...
                <div class="history-title">
                    Patient: ${item.patientName} ${advancedMeta ? `<span style="font-weight:400; color:var(--clr-text-muted); font-size:0.8rem">(${advancedMeta.slice(0, -3)})</span>` : ''}
                </div>
                <div class="history-details">
                    ...
                    ${item.files > 0 ? `<br><span style="color:var(--clr-text-muted);"><i class="ph ph-paperclip"></i> ${item.files} file(s) attached</span>` : ''}
                    ${item.notes ? `<br><strong style="color:var(--clr-primary); margin-top:4px; display:inline-block;">Suggestion:</strong> ${item.notes}` : ''}
                </div>
